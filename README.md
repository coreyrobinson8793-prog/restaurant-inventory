...
\- One-way PO state transitions (Draft → Finalized → Sent)
...
2\. Create a MySQL database named `restaurant\_inventory`

3\. Run the schema scripts (see `BUSINESS\_RULES.md` for schema details)
...
Full business rules documentation in `BUSINESS\_RULES.md`, covering authentication, inventory, suppliers, purchasing, and reporting.
...
Corey Robinson - pursuing a transition from 17 years in hospitality to software development. Currently completing an Associate's in Computer Technology at Trident Technical College.
